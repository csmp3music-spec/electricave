...
func _apply_environment_weather() -> void:
	...
	var fog_color := Color(0.70, 0.75, 0.82, 1.0)
	var fog_depth_begin := 180.0
	var fog_depth_end := 3200.0
	var aerial_perspective := 0.26
	var height_density := 0.02
	var sun_scatter := 0.42
	env.fog_enabled = true
	if state == "SNOW":
		base_density = 0.010 + snow_fog_boost * maxf(intensity, cloud_cover)
		volumetric_density = 0.012 + maxf(intensity, cloud_cover) * 0.009
		fog_color = Color(0.84, 0.87, 0.91, 1.0)
		fog_depth_begin = 60.0
		fog_depth_end = 950.0
		aerial_perspective = 0.48
		height_density = 0.10
		sun_scatter = 0.18
		env.background_color = env.background_color.lerp(Color(0.79, 0.82, 0.86, 1.0), 0.35)
	elif state == "RAIN":
		base_density = 0.007 + rain_fog_boost * maxf(intensity, cloud_cover)
		volumetric_density = 0.004 + maxf(intensity, cloud_cover) * 0.005
		fog_color = Color(0.55, 0.60, 0.67, 1.0)
		fog_depth_begin = 80.0
		fog_depth_end = 1500.0
		aerial_perspective = 0.38
		height_density = 0.07
		sun_scatter = 0.14
		...
		fog_color = Color(0.33, 0.37, 0.43, 1.0)
		fog_depth_begin = 45.0
		fog_depth_end = 700.0
		aerial_perspective = 0.62
		height_density = 0.12
		sun_scatter = 0.08
		...
		fog_color = Color(0.24, 0.27, 0.33, 1.0)
		fog_depth_begin = 35.0
		fog_depth_end = 520.0
		aerial_perspective = 0.7
		height_density = 0.16
		sun_scatter = 0.05
		...
		fog_color = Color(0.72, 0.77, 0.84, 1.0)
		fog_depth_begin = 220.0
		fog_depth_end = 3600.0
		aerial_perspective = 0.24 + humidity * 0.06
		height_density = 0.025
		sun_scatter = 0.46
	env.fog_density = base_density
	env.fog_light_color = fog_color
	env.fog_light_energy = lerpf(0.28, 0.12, storminess * 0.8 + intensity * 0.35)
	env.fog_sun_scatter = sun_scatter
	env.fog_aerial_perspective = aerial_perspective
	env.fog_sky_affect = lerpf(0.24, 0.44, cloud_cover * 0.7 + storminess * 0.3)
	env.fog_depth_begin = fog_depth_begin
	env.fog_depth_end = fog_depth_end
	env.fog_height = 28.0 + wetness * 16.0 + intensity * 10.0
	env.fog_height_density = height_density
	env.volumetric_fog_enabled = true
	env.volumetric_fog_density = volumetric_density
	env.volumetric_fog_albedo = fog_color.lerp(Color(0.96, 0.97, 1.0, 1.0), 0.24)
	env.volumetric_fog_emission = fog_color.lerp(Color(0.93, 0.95, 1.0, 1.0), 0.52)
	env.volumetric_fog_emission_energy = lerpf(0.02, 0.09, cloud_cover * 0.5 + wetness * 0.2 + storminess * 0.3)
	env.volumetric_fog_gi_inject = 0.55
	env.volumetric_fog_anisotropy = lerpf(0.20, 0.58, humidity * 0.45 + cloud_cover * 0.35 + wetness * 0.2)
	env.volumetric_fog_length = lerpf(220.0, 340.0, 1.0 - clampf(storminess * 0.55 + intensity * 0.35, 0.0, 1.0))
	env.volumetric_fog_detail_spread = 0.78
	env.volumetric_fog_ambient_inject = 0.62
	env.volumetric_fog_sky_affect = lerpf(0.18, 0.34, cloud_cover * 0.7 + storminess * 0.3)
